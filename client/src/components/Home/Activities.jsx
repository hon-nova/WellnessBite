...
const Activities = () => {
  ...
  const [likeStates,setLikeStates] =useState(displayArray.map(()=>({
    isLike: false,
    count: 0
  }))) 
  ...
  const saveActivity = async(activity_id)=>{
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8888/save-activity/${activity_id}`,{
        ...
        body: JSON.stringify({
          user_id:user_id,
          body_part:foundActivity.bodyPart,
          equipment: foundActivity.equipment,
          gif_url:foundActivity.gifUrl,
          name:foundActivity.name,
          target:foundActivity.target
        })
      })
              ...
              {displayArray.length > 0 &&
                displayArray.map((element,index) => (
                  <tr key={element.id}>
                    ...
                    style={{ backgroundColor:likeStates[element.id]?.isLike ? 'yellow': 'transparent' }}
