...
const Activities = () => {
  ...
  const handleClick = (activity_id)=>{
    setLikeState((prevState)=>{
      ...
      newArray[activity_id]={
        isLike: !newArray[activity_id]?.isLike
      }
      if(newArray[activity_id]?.isLike){
        saveActivity(activity_id)
      }
      return newArray
    })
    }
              ...
              {displayArray?.length > 0 &&
                displayArray.map((element,index) => (
                  <tr key={element.id}>
                    ...
                    style={{ backgroundColor: likeState[element.id]?.isLike ? 'yellow':'transparent' }}
